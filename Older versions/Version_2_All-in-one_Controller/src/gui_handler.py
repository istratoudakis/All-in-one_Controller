...
def launch_apps_gui():
        ...
        if idx < 3:
            ...
            def confirm():
                ...
                if path:
                    ...
                    # --- ΝΕΟΣ ΕΛΕΓΧΟΣ ΓΙΑ ΥΠΑΡΧΟΥΣΕΣ ΕΦΑΡΜΟΓΕΣ ---
                    ...
                    if app_name in ["discord", "obs", "obs studio", "zoom", "spotify"]:
                        ...
                        return # Σταματάει εδώ και δεν την προσθέτει
                    # ---------------------------------------------
    ...
    ttk.Button(bottom_frame, text="⚙️ Custom Buttons", command=toggle_custom).pack(side="left")
    
    dev_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(bottom_frame, text="🛠️ Dev Console", variable=dev_var).pack(side="left", padx=(10, 0))
    ...
    ttk.Button(bottom_frame, text="🚀 ΕΚΚΙΝΗΣΗ", command=start).pack(side="right")
